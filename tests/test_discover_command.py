...
class TestHandleDiscoverCommand:
    ...
    def test_handle_discover_command_table_output_happy_path(self, mock_handle_table, 
        ...
        # Act
        handle_discover_command(config, console, output_format="table", output_target="stdout")
        ...
        mock_handle_table.assert_called_once_with(mock_discovered_nodes, console, "stdout")
    ...
    def test_handle_discover_command_json_output_happy_path(self, mock_handle_json,
        ...
        # Act
        handle_discover_command(config, console, output_format="json", output_target="file")

        # Assert
        mock_handle_json.assert_called_once_with(mock_discovered_nodes, console, "file")

    def test_handle_discover_command_missing_proxmox_config(self):
        ...
        with pytest.raises(ConfigurationError) as exc_info:
            handle_discover_command(config, console)
        
        assert "Proxmox configuration is missing" in str(exc_info.value)

    @patch('k3s_deploy_cli.commands.discover_command.get_proxmox_api_client')
    def test_handle_discover_command_proxmox_connection_failure(self, mock_get_api_client):
...
class TestHandleJsonOutput:
    ...
    def test_handle_json_output_stdout(self):
        ...
        # Act
        _handle_json_output(discovered_nodes, console, "stdout")
    ...
    def test_handle_json_output_file(self, mock_update_config):
        ...
        # Act
        _handle_json_output(discovered_nodes, console, "file")

        # Assert
        expected_config_nodes = [{'vmid': 100, 'role': 'server'}]
        mock_update_config.assert_called_once_with(expected_config_nodes, console)

    def test_handle_json_output_empty_nodes(self):
...
class TestHandleTableOutput:
    ...
    def test_handle_table_output_with_nodes(self):
        ...
        # Act
        _handle_table_output(discovered_nodes, console, "stdout")

        # Assert
        assert console.print.call_count >= 3  # Table + 2 info messages

    def test_handle_table_output_empty_nodes(self):
        ...
        # Assert
        console.print.assert_called()  # Should still print empty table

    def test_handle_table_output_different_target(self):
        ...
        # Act & Assert
        with pytest.raises(ConfigurationError) as exc_info:
            _handle_table_output(discovered_nodes, console, "file")
...
class TestUpdateConfigFileWithNodes:
    ...
    def test_update_config_file_with_nodes_successful(self, mock_exists, mock_file):
        ...
        # Act
        _update_config_file_with_nodes(discovered_nodes, console)

        # Assert
        assert mock_file.call_count >= 3  # Read original, write backup, write updated
        console.print.assert_called()  # Success message displayed

    @patch('builtins.open', new_callable=mock_open)
    @patch('k3s_deploy_cli.commands.discover_command.Path.exists')
    def test_update_config_file_missing_config(self, mock_exists, mock_file):
        ...
        # Assert
        console.print.assert_called()  # Should display error message

    @patch('builtins.open', new_callable=mock_open, read_data='invalid json')
    @patch('k3s_deploy_cli.commands.discover_command.Path.exists')
    def test_update_config_file_invalid_json(self, mock_exists, mock_file):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('k3s_deploy_cli.commands.discover_command.Path.exists')
    def test_update_config_file_backup_failure(self, mock_exists, mock_file):
    ...
    @patch('builtins.open', new_callable=mock_open, read_data='{"proxmox": {"host": "test.com"}}')
    @patch('k3s_deploy_cli.commands.discover_command.Path.exists')
    def test_update_config_file_write_failure(self, mock_exists, mock_file):
    ...
    @patch('k3s_deploy_cli.commands.discover_command.Path.exists')
    def test_update_config_file_overwrites_existing_nodes(self, mock_exists, mock_file):
    ...
    def test_update_config_file_empty_nodes_list(self):
        ...
        # Assert
        console.print.assert_called()  # Should still attempt update
...
class TestDiscoverCommandIntegration:
    ...
    def test_discover_command_full_flow_table_format(self, mock_discover_nodes, mock_get_api_client):
        ...
        assert console.print.call_count > 0  # Table and info messages displayed
    ...
    def test_discover_command_parameter_validation(self):
        ...
        for output_format, output_target in valid_combinations:
            with patch('k3s_deploy_cli.commands.discover_command.get_proxmox_api_client') as mock_client:
                with patch('k3s_deploy_cli.commands.discover_command.discover_k3s_nodes') as mock_discover:
                    ...
                    handle_discover_command(config, console, output_format, output_target)
...
class TestDiscoverCommandErrorScenarios:
    ...
    def test_discover_command_authentication_failure(self, mock_get_api_client):
        ...
        # Arrange
        config = {'proxmox': {'host': 'pve.com', 'user': 'invalid', 'password': 'wrong'}}
        console = MagicMock(spec=Console)
    ...
    def test_discover_command_malformed_config(self):
    ...
    def test_discover_command_unicode_vm_names(self, mock_discover_nodes, mock_get_api_client):
        ...
        mock_get_api_client.assert_called_once()
        mock_discover_nodes.assert_called_once()
        console.print.assert_called()  # Should display table with Unicode names
