...
class TestCommandDispatch:
    ...
    @patch('k3s_deploy_cli.main.handle_info_command')
    def test_dispatch_info_command(self, mock_handle_info):
        ...
        # Assert
        mock_handle_info.assert_called_once_with(config, console, discover=False)

    @patch('k3s_deploy_cli.main.handle_info_command')
    def test_dispatch_info_command_with_discover(self, mock_handle_info):
        ...
        # Assert
        mock_handle_info.assert_called_once_with(config, console, discover=True)

    @patch('k3s_deploy_cli.main.handle_discover_command')
    def test_dispatch_discover_command(self, mock_handle_discover):
        ...
        # Assert
        mock_handle_discover.assert_called_once_with(config, console, 'table', 'stdout')

    @patch('k3s_deploy_cli.main.handle_discover_command')
    def test_dispatch_discover_command_json_format(self, mock_handle_discover):
        ...
        # Assert
        mock_handle_discover.assert_called_once_with(config, console, 'json', 'file')
    ...
    def test_dispatch_command_missing_discover_attr(self):
        ...
        # Act & Assert - Should handle missing attribute gracefully
        with patch('k3s_deploy_cli.main.handle_info_command') as mock_handle_info:
            _dispatch_command(args, config, console)
            mock_handle_info.assert_called_once_with(config, console, discover=False)
